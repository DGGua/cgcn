...
void rerArray(int *featrue_data, int featrue_length, int *weight_array, int output_size, int node_cnt, int *output_data)
{
#line 12 "/home/ubuntu/VitisCodes/test2/solution1/directives.tcl"
#pragma HLSDIRECTIVE DATAFLOW
# 20 "test2/systolic.cpp"

    hls::stream<int> property_input[4][output_size];
    hls::stream<int> weight_input[4][output_size];
    hls::stream<int> output[4][output_size];
    ...
    input_turn:
        ...
        input_property:
            for (int row = 0; row < 4; row++)
            {
                ...
                VITIS_LOOP_45_1: for (int col = 0; col < output_size; col++)
                {
        ...
        input_weight:
            for (int col = 0; col < output_size; col++)
            {
                ...
                VITIS_LOOP_54_2: for (int row = 0; row < 4; row++)
                {
                    weight_input[row][col].write(weight_val);
                }
            }
        }
        VITIS_LOOP_60_3: for (int col = 0; col < output_size; col++)
        {
            VITIS_LOOP_62_4: for (int row = 0; row < 4; row++)
            {
                buff[row][col] = output[row][col].read();
            }
        }
        VITIS_LOOP_67_5: for (int row = 0; row < 4; row++)
        {
