...
input_batch:
    ...
    input_turn_property:
        ...
        input_property:
            for (int row = 0; row < ARRAY_HEIGHT; row++)
            {
                int featrue_val = featrue_data[(batch * ARRAY_HEIGHT + row) * featrue_length + turn];
                for (int col = 0; col < output_size; col++)
                {
    ...
    input_turn_weight:
        ...
        for (int row = 0; row < ARRAY_HEIGHT; row++)
        {
            memcpy(output_data + (batch * ARRAY_HEIGHT + row) * output_size, buff[row], output_size * sizeof(int));
        }
    }
}
